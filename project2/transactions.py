                ...
                while i < numrows:
                    try:
                        if(len(res[i]['user_apikey']) >= 128):
                            ots = time.mktime(datetime.datetime.strptime(str(res[i]['user_lasttransaction']), "%Y-%m-%d %H:%M:%S").timetuple())
                            response = requests.get('https://HIDDEN_FOR_PRIVACY_REASONS/public/v1/user/transactions/?closed_at__gte=%s' % (int(ots)),
                                ...
                                if lenlist > 0:
                                    ...
                                    while ii < lenlist:
                                        if int(list_[ii]['closed_at']) > int(ots):
                                            tsum = '{:.8f}'.format(float(list_[ii]['sum'])).rstrip('0')
                                            tbalance = '{:.8f}'.format(float(list_[ii]['balance'])).rstrip('0')
                                            ttype = {1: 'replenish', 2: 'withdrawal', 5: 'transfer'}
                                            if list_[ii]['type'] == 1:
                                                await bot.send_message(res[i]['user_tgid'], "↘️New transaction!\nType: %s\nSum: %s %s\nBalance: %s %s" % (ttype.get(list_[ii]['type']), tsum, list_[ii]['currency'], tbalance, list_[ii]['currency']))
                                            elif list_[ii]['type'] == 2:
                                                await bot.send_message(res[i]['user_tgid'], "↗️New transaction!\nType: %s\nSum: %s %s\nBalance: %s %s" % (ttype.get(list_[ii]['type']), tsum, list_[ii]['currency'], tbalance, list_[ii]['currency']))
                                            elif list_[ii]['type'] == 5:
                                                await bot.send_message(res[i]['user_tgid'], "🔄New transaction!\nType: %s\nSum: %s %s\nBalance: %s %s" % (ttype.get(list_[ii]['type']), tsum, list_[ii]['currency'], tbalance, list_[ii]['currency']))
                                        ii += 1
                                        time.sleep(0.1)
                                    lo_ts = datetime.datetime.utcfromtimestamp(int(list_[0]['closed_at'])).strftime('%Y-%m-%d %H:%M:%S')
                                    cursor.execute("UPDATE users SET user_lasttransaction='%s' WHERE user_id=%s" % (lo_ts, res[i]['user_id']))
